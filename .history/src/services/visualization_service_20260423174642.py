...
def generar_imagen_disponibilidad_semana(user_id: int, fecha_inicio: datetime) -> str:
    ...
    try:
        ...
        for idx in range(7):
            ...
            horas = list(range(HORAS_INICIO, HORAS_FIN))
            ax.set_xticks(horas)
            ax.set_xticklabels([f"{h:02d}:00" for h in horas], fontsize=7)
            ax.grid(axis='x', alpha=0.3, linestyle='--')
...
def generar_imagen_disponibilidad_semana_24h(user_id: int, fecha_cualquiera: datetime) -> str:
    ...
    try:
        ...
        # Crear figura con subplots (7 días)
        fig, axes = plt.subplots(7, 1, figsize=(20, 14))
        fig.patch.set_facecolor('#f8f9fa')
        
        titulo = f"Disponibilidad Semanal - Semana del {fecha_lunes.strftime('%d de %B')} al {(fecha_lunes + timedelta(days=6)).strftime('%d de %B')}"
        fig.suptitle(titulo, fontsize=16, fontweight='bold')
        ...
        for idx in range(7):
            ...
            # Configurar ejes
            ax.set_title(f"{dias_semana[idx]} - {fecha_dia.strftime('%d/%m/%Y')}", 
