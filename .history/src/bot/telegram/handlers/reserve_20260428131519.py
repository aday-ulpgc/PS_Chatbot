...
async def handle_alternative_time_selection(
    ...
    try:
        ...
        for idx, (fecha, hora) in enumerate(alternatives):
            ...
            # Crear callback con la hora alternativa
            callback_data = f"alt_time_{idx}_{fecha_iso}_{hora.replace(':', '')}"
...
async def handle_alternative_time_selection_callback(
    ...
    try:
        # Parsear el callback: alt_time_0_2026-06-12_0600
        parts = query.data.split("_")
        if len(parts) < 4 or parts[0] != "alt":
            return
        
        idx = int(parts[2])
        fecha_iso = parts[3]  # YYYY-MM-DD
        hora_str = parts[4]   # HHMM
        
        # Convertir hora de HHMM a HH:MM
        hora_formatted = f"{hora_str[:2]}:{hora_str[2:]}"
        
        # Convertir fecha de ISO a DD/MM/YYYY para mostrar
