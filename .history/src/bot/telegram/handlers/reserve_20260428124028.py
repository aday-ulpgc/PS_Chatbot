...
async def handle_action_my_appointments(
    ...
    try:
        ...
        with get_session() as session:
            ...
            # Enviar primer bloque con el mensaje editado
            bloque_texto = "📋 *Mis citas* (grupo 1 de {})\n\n".format(len(bloques))
            ...
            # Enviar bloques adicionales como mensajes nuevos (si hay)
            for num_bloque, bloque in enumerate(bloques[1:], 2):
                bloque_texto = f"📋 *Mis citas* (grupo {num_bloque} de {len(bloques)})\n\n"
                for idx, cita in enumerate(bloque):
                    fecha_str = cita.FECHA.strftime("%d/%m/%Y %H:%M")
                    bloque_texto += f"{emojis[idx]} *{fecha_str}* - {cita.DESCRIPCION or 'Sin descripción'}\n"
