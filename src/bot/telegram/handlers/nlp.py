...
async def _reply_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE, texto_respuesta: str):
    ...
    if user_mode == MODO_AUDIO:
        async with send_action_while_thinking(context.bot, update.effective_chat.id, constants.ChatAction.RECORD_VOICE):
            try:
...
async def _handle_settings_intent(update: Update, context: ContextTypes.DEFAULT_TYPE, agente: dict, texto_respuesta: str):
...
async def _handle_booking_intent(update: Update, context: ContextTypes.DEFAULT_TYPE, agente: dict, texto_respuesta: str):
...
async def _handle_availability_intent(update: Update, context: ContextTypes.DEFAULT_TYPE, agente: dict, texto_respuesta: str):
    if agente.get("estado") == "listo_para_consultar_disponibilidad":
        await _reply_to_user(update, context, texto_respuesta)
    else:
        await _reply_to_user(update, context, texto_respuesta)

async def _handle_appointments_intent(update: Update, context: ContextTypes.DEFAULT_TYPE, agente: dict, texto_respuesta: str):
...
async def _handle_cancel_intent(update: Update, context: ContextTypes.DEFAULT_TYPE, agente: dict, texto_respuesta: str):
...
async def _handle_modify_intent(update: Update, context: ContextTypes.DEFAULT_TYPE, agente: dict, texto_respuesta: str):
...
async def handle_texto_libre(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with send_action_while_thinking(context.bot, update.effective_chat.id, constants.ChatAction.TYPING):
