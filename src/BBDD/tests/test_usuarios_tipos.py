...
if response.status_code == 201:
    ...
    print(
        f"✅ Usuario individual creado (ID: {id_usuario_i}, TIPO: {usuario_i['TIPO']})"
    )
else:
...
contacto_i_data = {"NOMBRE": "Dr. García", "EMAIL": "dr.garcia@clinic.com"}

response = requests.post(
    f"{BASE_URL}/usuarios/{id_usuario_i}/contactos", json=contacto_i_data
)
if response.status_code == 201:
...
if response.status_code == 200:
    ...
    for c in citas:
        print(
            f"   - ID: {c['ID_CITA']}, Duración: {c.get('DURACION')} min, Desc: {c.get('DESCRIPCION')}"
        )
else:
    print(f"❌ Error: {response.text}")
...
if response.status_code == 201:
    ...
    print(
        f"✅ Usuario corporativo creado (ID: {id_usuario_c}, TIPO: {usuario_c['TIPO']})"
    )
else:
    print(f"❌ Error: {response.text}")
    id_usuario_c = None
...
if id_usuario_c:
    ...
    empleado_data = {
        "NOMBRE": "Dr. Pérez",
        "TIPO": "E",  # Empleado
        "CONTRASENA_CORP": "pass_empleado123",  # Requerido para empleados
    }

    response = requests.post(
        f"{BASE_URL}/usuarios/{id_usuario_c}/empleados", json=empleado_data
    )
    if response.status_code == 201:
        ...
        print(f"⚠️  Error al crear empleado: {response.status_code}")
...
print(
    f"✅ Usuario Corporativo (TIPO=C): ID {id_usuario_c if id_usuario_c else 'ERROR'}"
)
if id_empleado:
    print(f"✅ Empleado: ID {id_empleado}")
else:
    print("⚠️  Empleado: No creado")

print("\n📊 QUERIES EN DBEAVER:")
print("\n1️⃣  Ver citas del usuario individual (con DURACION):")
print(
    f"    SELECT ID_CITA, DESCRIPCION, DURACION FROM CITAS_IND WHERE ID_USUARIO = {id_usuario_i} ORDER BY FECHA DESC;"
)

print("\n2️⃣  Ver usuarios de ambos tipos:")
print(
    "    SELECT ID_USUARIO, NOMBRE, TIPO FROM USUARIOS WHERE TIPO IN ('I', 'C') ORDER BY ID_USUARIO DESC LIMIT 5;"
)

if id_usuario_c:
    print("\n3️⃣  Ver empleados (si existen):")
    print(
        f"    SELECT ID_EMPLEADO, NOMBRE, TIPO FROM EMPLEADOS WHERE ID_USUARIO_ADM = {id_usuario_c};"
    )
