...
def verificar_citas_especificas():
    ...
    try:
        ...
        for cita in citas_data:
            ...
            dia_fin = (dia_inicio + timedelta(days=1)).replace(
            ...
            if events:
                print(
                    f"✅ Cita ID {cita['id']} ({fecha_obj.strftime('%d/%m/%Y %H:%M')}) ENCONTRADA en Google Calendar:"
                )
                for event in events:
                    print(f"   - {event.get('summary', 'Sin nombre')}")
            else:
                print(
                    f"❌ Cita ID {cita['id']} ({fecha_obj.strftime('%d/%m/%Y %H:%M')}) NO encontrada en Google Calendar"
                )
